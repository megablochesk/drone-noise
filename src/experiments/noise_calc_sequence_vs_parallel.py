...
@njit(parallel=True)
def calculate_cells_noise_parallel(cell_northings, cell_eastings, drone_northings, drone_eastings):
...
def calculate_cells_noise_sequential(cell_northings, cell_eastings, drone_northings, drone_eastings):
    num_cells = len(cell_northings)
    num_drones = len(drone_northings)
    total_noise_result = np.zeros(num_cells, dtype=np.float64)
    ...
    def calculate_noise_cells(self, use_parallel=True):
        ...
        with tqdm(total=len(self.drone_location_history), desc="Calculating Noise Matrix", unit="iteration") as pbar:
            for drone_locations in self.drone_location_history:
                ...
                if use_parallel:
                    total_noise = calculate_cells_noise_parallel(
                        cell_northings,
                        cell_eastings,
                        drone_northings,
                        drone_eastings
                    ...
                    total_noise = calculate_cells_noise_sequential(
                        cell_northings,
                        cell_eastings,
                        drone_northings,
                        drone_eastings
        ...
        print(f"Iteration took {elapsed_time:.4f} seconds (Parallel={use_parallel})")
